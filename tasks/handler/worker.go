...
func handleMsgEvent(ctx context.Context, db *sqlx.DB, rp *redis.Pool, event *MsgEvent) error {
	...
	tickets, err := models.LoadOpenTicketsForContact(ctx, db, org.OrgID(), modelContact)
	if err != nil {
		return errors.Wrapf(err, "unable to look up open tickets for contact")
	}
	for _, ticket := range tickets {
		ticket.ForwardIncoming(ctx, db, org, modelContact, event.MsgUUID, event.Text, event.Attachments)
	}
