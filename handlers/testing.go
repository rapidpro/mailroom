...
func GetTestSessions() ([]*models.Session, error) {
	...
	// load our contacts
	contacts, err := models.LoadContacts(ctx, db, sessionAssets, org, []flows.ContactID{Cathy, Bob, Evan})
	if err != nil {
		return nil, err
	}
	...
	for i, contact := range contacts {
		...
		// create our trigger
		trigger := triggers.NewManualTrigger(org.Env(), contact, flowRef, nil, time.Now())
