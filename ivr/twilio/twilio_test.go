...
func TestResponseForSprint(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			`<Response><Gather numDigits="1" timeout="120" action="http://temba.io/resume?session=1"><Say>enter a number</Say></Gather><Redirect>http://temba.io/resume?session=1&amp;timeout=true</Redirect></Response>`,
		},
		{
			...
			`<Response><Gather finishOnKey="#" timeout="120" action="http://temba.io/resume?session=1"><Say>enter a number, then press #</Say></Gather><Redirect>http://temba.io/resume?session=1&amp;timeout=true</Redirect></Response>`,
		},
		{
			...
			`<Response><Say>say something</Say><Record action="http://temba.io/resume?session=1"></Record></Response>`,
		},
	}
