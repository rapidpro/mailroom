...
func (b *FlowStartBatch) ParentSummary() json.RawMessage { return b.b.ParentSummary }
...
type FlowStart struct {
	s struct {
		...
		ParentSummary json.RawMessage    `json:"parent_summary,omitempty" db:"parent_summary"`
	}
}
...
func (s *FlowStart) ParentSummary() json.RawMessage { return s.s.ParentSummary }
...
func GetFlowStartAttributes(ctx context.Context, db Queryer, orgID OrgID, startID StartID) (*FlowStart, error) {
	start := &FlowStart{}
	err := db.GetContext(ctx, &start.s, `SELECT id, uuid, flow_id, extra, parent_summary FROM flows_flowstart WHERE id = $1 AND is_active = TRUE`, startID)
	if err != nil {
