...
func NewBroadcastFromEvent(ctx context.Context, tx DBorTxx, oa *OrgAssets, event *events.BroadcastCreatedEvent) (*Broadcast, error) {
...
func MarkBroadcastSent(ctx context.Context, db DBorTxx, id BroadcastID) error {
...
func MarkBroadcastFailed(ctx context.Context, db DBorTxx, id BroadcastID) error {
	_, err := db.ExecContext(ctx, `UPDATE msgs_broadcast SET status = 'S', modified_on = now() WHERE id = $1`, id)
...
func InsertChildBroadcast(ctx context.Context, db DBorTxx, parent *Broadcast) (*Broadcast, error) {
	child := NewBroadcast(
		parent.OrgID,
		parent.Translations,
