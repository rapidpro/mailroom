...
func TestTicketers(t *testing.T) {
	...
	ticketer.UpdateConfig(ctx, rt.DB, map[string]string{"new-key": "foo"}, map[string]bool{"push_id": true})
...
func TestTickets(t *testing.T) {
	...
	tk2, err := models.LookupTicketByExternalID(ctx, rt.DB, testdata.Zendesk.ID, "EX7869")
	assert.NoError(t, err)
...
func TestUpdateTicketConfig(t *testing.T) {
	...
	ticket := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.DefaultTopic, "Where my shoes", "123", time.Now(), nil)
	modelTicket := ticket.Load(rt)

	// empty configs are null
	assertdb.Query(t, rt.DB, `SELECT count(*) FROM tickets_ticket WHERE config IS NULL AND id = $1`, ticket.ID).Returns(1)

	models.UpdateTicketConfig(ctx, rt.DB, modelTicket, map[string]string{"foo": "2352", "bar": "abc"})

	assertdb.Query(t, rt.DB, `SELECT count(*) FROM tickets_ticket WHERE config='{"foo": "2352", "bar": "abc"}'::jsonb AND id = $1`, ticket.ID).Returns(1)

	// updates are additive
	models.UpdateTicketConfig(ctx, rt.DB, modelTicket, map[string]string{"foo": "6547", "zed": "xyz"})

	assertdb.Query(t, rt.DB, `SELECT count(*) FROM tickets_ticket WHERE config='{"foo": "6547", "bar": "abc", "zed": "xyz"}'::jsonb AND id = $1`, ticket.ID).Returns(1)
}
...
func TestTicketsAssign(t *testing.T) {
	...
	ticket1 := testdata.InsertClosedTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.DefaultTopic, "Where my shoes", "123", nil)
	modelTicket1 := ticket1.Load(rt)

	ticket2 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Zendesk, testdata.DefaultTopic, "Where my pants", "234", time.Now(), nil)
	modelTicket2 := ticket2.Load(rt)

	// create ticket already assigned to a user
	ticket3 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Zendesk, testdata.DefaultTopic, "Where my glasses", "", time.Now(), testdata.Admin)
...
func TestTicketsAddNote(t *testing.T) {
	...
	ticket2 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Zendesk, testdata.DefaultTopic, "Where my pants", "234", time.Now(), testdata.Agent)
	modelTicket2 := ticket2.Load(rt)
...
func TestTicketsChangeTopic(t *testing.T) {
	...
	ticket1 := testdata.InsertClosedTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.SalesTopic, "Where my shoes", "123", nil)
	modelTicket1 := ticket1.Load(rt)

	ticket2 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Zendesk, testdata.SupportTopic, "Where my pants", "234", time.Now(), nil)
	modelTicket2 := ticket2.Load(rt)

	ticket3 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Zendesk, testdata.DefaultTopic, "Where my pants", "345", time.Now(), nil)
	modelTicket3 := ticket3.Load(rt)
...
func TestCloseTickets(t *testing.T) {
	...
	ticket1 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.DefaultTopic, "Where my shoes", "123", time.Now(), nil)
	modelTicket1 := ticket1.Load(rt)
	...
	ticket3 := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.DefaultTopic, "Where my shoes", "123", time.Now(), nil)
	modelTicket3 := ticket3.Load(rt)
...
func TestTicketRecordReply(t *testing.T) {
	...
	ticket := testdata.InsertOpenTicket(rt, testdata.Org1, testdata.Cathy, testdata.Mailgun, testdata.DefaultTopic, "Where my shoes", "123", openedOn, nil)
